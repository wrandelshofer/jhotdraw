...
public class PaletteFontChooserUI extends FontChooserUI {
    ...
    private void updateCollectionList() {
        isUpdating++;
        JList<MutableTreeNode> list = selectionPanel.getCollectionList();
        DefaultListModel<MutableTreeNode> lm = (DefaultListModel<MutableTreeNode>) list.getModel();
        lm.removeAllElements();
        FontChooserModel model = fontChooser.getModel();
        Object parent = model.getRoot();
        for (int i = 0, n = model.getChildCount(parent); i < n; i++) {
            lm.addElement((MutableTreeNode)model.getChild(parent, i));
        }
    ...
    private void updateFamilyList() {
        isUpdating++;
        JList<MutableTreeNode> list = selectionPanel.getFamilyList();
        FontChooserModel model = fontChooser.getModel();
        ...
        if (newParent != familyListParent) {
            ...
            if (familyListParent != null) {
                for (int i = 0, n = model.getChildCount(familyListParent); i < n; i++) {
                    lm.addElement((MutableTreeNode)model.getChild(familyListParent, i));
                }
            }
        }
    ...
    private void updateFaceList() {
        isUpdating++;
        JList<MutableTreeNode> list = selectionPanel.getFaceList();
        FontChooserModel model = fontChooser.getModel();
        ...
        if (newParent != faceListParent) {
            ...
            if (faceListParent != null) {
                for (int i = 0, n = model.getChildCount(faceListParent); i < n; i++) {
                    lm.addElement((MutableTreeNode)model.getChild(faceListParent, i));
                }
            }
        }
