...
public class BiArc {
     ...
     */
    public BiArc(Point2D p1, Point2D t1, Point2D p2, Point2D t2, Point2D tp) {
        ...
        var tl1 = Line.createPerpendicularAt(p1, p1.add(t1));
        var tl2 = Line.createPerpendicularAt(p2, p2.add(t2));

        // Calculate the perpendicular bisector of P1T and P2T
        var P1T2 = (p1.add(tp)).multiply(0.5);
        var pbP1T = Line.createPerpendicularAt(P1T2, tp);

        var P2T2 = (p2.add(tp)).multiply(0.5);
        ...
        // Calculate the radii
        var r1 = (C1.subtract(p1)).magnitude();
        var r2 = (C2.subtract(p2)).magnitude();

        // Calculate start and sweep angles
        var startVector1 = p1.subtract(C1);
        var endVector1 = tp.subtract(C1);
        ...
        var startVector2 = tp.subtract(C2);
        var endVector2 = p2.subtract(C2);
