...
public interface ShapeableFigure extends Figure {
    ...
    default void applyShapeableProperties(RenderContext ctx, @NonNull Path node, @NonNull Bounds b) {
        ...
        FXPathBuilder builder = new FXPathBuilder(node.getElements());
        try {
            Shapes.buildFromSvgString(builder, content);
        } catch (ParseException ex) {
            Logger.getLogger(ShapeableFigure.class.getName()).warning("Illegal path: " + content);
        }
        builder.getElements(); // will update the provided elements


        javafx.geometry.Rectangle2D shapeBounds = getStyled(SHAPE_BOUNDS);

        final Bounds srcBounds = shapeBounds == null || FXGeom.isEmpty(shapeBounds) ? node.getLayoutBounds() : FXGeom.getBounds(shapeBounds);
