...
private final static int LASTEDGE_POINTER_FIELD=1;
    ...
    public void setVertexCount(int newValue) {
        if (newValue<vertexCount)throw new IllegalArgumentException("can only add vertices:"+newValue);
    ...
    public int getNext(int vi, int i) {
        if (vi<0||vi>=getVertexCount()) throw new IllegalArgumentException("vi:"+i);
        if (i<0||i>=getNextCount(vi)) throw new IllegalArgumentException("i:"+i);
