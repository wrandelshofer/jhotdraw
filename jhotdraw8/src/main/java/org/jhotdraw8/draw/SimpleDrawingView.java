...
import org.jhotdraw8.draw.handle.MultipleSelectionMoveHandle;
import org.jhotdraw8.draw.handle.MultipleSelectionOutlineHandle;
...
public class SimpleDrawingView extends AbstractDrawingView {
  ...
  /**
   * This is the JavaFX Node which is used to represent this drawing view. in a
   * JavaFX scene graph.
   */
  ...
  /**
   * Maps JavaFX nodes to a figure. Note that the DrawingView may contain JavaFX
   * nodes which have no mapping. this is usually the case, when a Figure is
   * represented by multiple nodes. Then only the parent of these nodes is
   * associated with the figure.
   */
  private final Map<Figure, Node> figureToNodeMap = new IdentityHashMap<>();
  /**
   * This is the set of figures which are out of sync with their JavaFX node. We
   * do not wrap the IdentityHashMap into a Set to avoid an additional level of
   * indirection.
   */
  private final Set<Figure> dirtyFigureNodes = new HashSet<>();
  /**
   * This is the set of handles which are out of sync with their JavaFX node. We
   * do not wrap the IdentityHashMap into a Set to avoid an additional level of
   * indirection.
   */
  ...
  /**
   * The set of all secondary handles. One handle at a time may create secondary
   * handles.
   */
...
//    drawingPane.setTranslateX(max(0, -x));
  ...
  /**
   * Returns true if the node contains the specified point within a a tolerance.
   *
   * @param node The node
   * @param point The point in local coordinates
   * @param tolerance The maximal distance the point is allowed to be away from
   * the
